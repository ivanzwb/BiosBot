...
- WebSocket 实时推送任务状态、新消息、知识库导入进度、**执行步骤**等事件（`ws://host:port/ws`）。
- **执行步骤实时推送**：后端在处理用户请求时，通过 `step:update` 事件实时推送各执行阶段（意图识别、路由规划、Agent 调用、结果汇总），前端可实时展示处理进度。
...
- **对话页（ChatPage）**：
  ...
  - **执行步骤指示器**：发送消息时实时显示后端处理进度（🔍 意图识别 → 🗂️ 路由规划 → 🤖 Agent 调用 → 📊 结果汇总）。
  - 发送消息自动创建对话（无需预先新建），首条消息后自动调用 LLM 生成标题。
  - WebSocket + 轮询双通道（race 模式）实时获取任务结果和执行步骤。
...
│   │   │   └── ws.service.ts         # WebSocket 服务（广播事件：task:update/message:new/step:update）
...
│   │   │   ├── agent.types.ts        # DomainAgent / AgentInput / AgentOutput / ExecutionStep / StepCallback
...
│   │   │   │       └── execution_steps_indicator.dart # 执行步骤指示器（实时显示后端处理进度）
│   │   │   ├── agents/
...
│   │   │   ├── message.dart
│   │   │   ├── task.dart
│   │   │   └── execution_step.dart    # 执行步骤模型（StepType/StepStatus/ExecutionStep）
│   │   └── utils/
...
- **backend/src/services**：承载主要业务逻辑——`intent.service`（意图识别）、`router.service`（路由调度）、`aggregation.service`（响应聚合）、`chat.service`（对话/消息 CRUD）、`task.service`（任务生命周期）、`ws.service`（WebSocket 事件广播，含 `step:update` 执行步骤实时推送）。
- **backend/src/agents**：Agent 共享代码和代码驱动的 Agent。
  ...
  - `proxy-agent/`：中枢代理 Agent，从 `agent_model_mapping` 读取可配置的 `classifyPrompt`、`aggregatePrompt`、`temperature`；在工作流执行时通过 `onStep` 回调实时推送执行步骤（意图识别、路由规划、Agent 调用、结果聚合）。
